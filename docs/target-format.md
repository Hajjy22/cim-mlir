...
| `reduce_max.latency_ns`, `reduce_max.energy_pj` | Cost of one chained compare-and-select inside `cim.reduce_max` — a MaxPool's kernel window, charged once per `cimrt_reduce_max` call an *N*-operand `cim.reduce_max` lowers to (*N*-1 calls), exactly like `reduce_partial` above. Required for the same reason. **A separate entry rather than reusing `reduce_partial`**: a compare-and-select is a different datapath element from an adder on a digital target, and has no in-array realization at all on an analog one, so billing a pooling window to the adder's entry would publish a number that reads as measured and is not. The shipped targets deliberately give the two entries *different* values, so a miswiring shows up as a wrong number instead of an identical one. Note the values themselves are marked in each file as placeholders — no hardware measurement exists for this step. |
