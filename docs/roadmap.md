...
- `Relu` on a conv chain's own *final* layer output, or directly before a conv-pool
  chain's own trailing (pre-matmul-bridge) pool — every other position now accepted.
