...
- `Relu` directly before a conv-pool chain's own trailing (pre-matmul-bridge)
  pool, or between matmul layers — every other Relu position, including both
  final-layer cases (unpooled and pooled) and the plain (unpooled)
  conv-to-matmul bridge, now accepted.
