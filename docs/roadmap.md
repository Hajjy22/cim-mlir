...
- `Relu` on a *conv-pool* chain's (`load_conv_pool_chain`) own final layer output
  — `load_conv_chain`'s identical unpooled case is now accepted — directly before
  a conv-pool chain's own trailing (pre-matmul-bridge) pool, or between matmul
  layers — every other position, including the plain (unpooled) conv-to-matmul
  bridge, now accepted.
