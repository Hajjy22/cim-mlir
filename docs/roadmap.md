...
- `Relu` on a conv chain's own *final* layer output, directly before a conv-pool
  chain's own trailing (pre-matmul-bridge) pool, or between matmul layers — every
  other position, including the plain (unpooled) conv-to-matmul bridge, now accepted.
