...
own last layer and the `Transpose` that starts it, or directly on a conv chain's own final
layer output (with or without pooling elsewhere in the chain) — the graph's own declared
output IS that Relu's own output, reading the last conv's "Y" directly, with nothing else
reading it. A `Relu` anywhere else
...
Still refused: a `Relu` directly before a conv-pool chain's own trailing pool (the one
sitting right before the conv-to-matmul bridge, when a pool is also present there); pooling
directly after a conv chain's own last layer (regardless of Relu — a long-standing,
unrelated restriction of its own); and a `Relu` between matmul layers.
